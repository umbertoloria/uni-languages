...
\begin{math}
a^{n + 1} \in L(A_p)
\Rightarrow \hat\delta(p, a^{n + 1}) = q
\Rightarrow \hat\delta(p, a^n) = p
\Rightarrow a^n \in L(A_p) \\
\Rightarrow \text{ (per ipotesi induttiva) } n \text{ pari}
\Rightarrow n + 1 \text{ dispari}
\end{math}
\\
\\
...
\Rightarrow \hat\delta(p, a^{n + 1}) = \delta(\hat\delta(p, a^n), a) = \delta(q, a) = p
